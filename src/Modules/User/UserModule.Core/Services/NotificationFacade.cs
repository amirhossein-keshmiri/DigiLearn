...
using UserModule.Core.Queries.Notifications.GetById;
...
namespace UserModule.Core.Services
{
  public class NotificationFacade : INotificationFacade
  {
    ...
    public async Task<OperationResult> SeenAll(SeenAllNotificationsCommand command)
    {
      return await _mediator.Send(command);
    }
    ...
    public async Task<NotificationDto?> GetNotification(Guid notificationId, Guid userId)
    {
      return await _mediator.Send(new GetNotificationByIdQuery(notificationId, userId));
    }
  }
}
